...
def set_reg(offset,prefix,name,regmap):
  ...
  if (type(val) is list):
    for i,v in enumerate(val):
      print offset+regmap[name]+i, v, "#", prefix+name+"["+str(i)+"]"
  else:
...
for i,m in enumerate([mode1,mode2]):
  ...
  regmap_emode = {
    ...
    'out_couple' : get_reg_info(regmap_cryomodule,[cav_num, i],"out_couple_out_coupling")["base_addr"]
...
if (prng_seed is not None):
  ...
  hf = sha1()
  hf.update(prng_seed)
  push_seed(get_reg_info(regmap_cryomodule,[cav_num],"prng_iva")["base_addr"],hf)
  push_seed(get_reg_info(regmap_cryomodule,[cav_num],"prng_ivb")["base_addr"],hf)
  print "%d 1  # turn on PRNG"%(get_reg_info(regmap_cryomodule,[cav_num],"prng_random_run")["base_addr"])
  # push_seed(get_reg_info(regmap_cryomodule,[],"cav4_mech_prng_iva")["base_addr"],hf)
  # push_seed(get_reg_info(regmap_cryomodule,[],"cav4_mech_prng_ivb")["base_addr"],hf)
  # print "%d 1  # turn on PRNG"%(get_reg_info(regmap_cryomodule,[],"cav4_mech_prng_random_run")["base_addr"])
...
get_ctl_reg_and_set('dsp_phase_step', dds_phstep)
get_ctl_reg_and_set('dsp_modulo', dds_modulo)
