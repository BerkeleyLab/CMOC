...
wire start_outer;
reg_delay #(.dw(1), .len(0)) start_outer_g(.clk(clk2x), .gate(1'b1), .din(start), .dout(start_outer));
wire start_eig;
reg_delay #(.dw(1), .len(1)) start_eig_g(.clk(clk2x), .gate(1'b1), .din(start), .dout(start_eig));
...
wire res_clip;
cav4_mech #(.n_mech_modes(n_mech_modes)) cav4_mech // auto clk2x
  (.clk(clk2x),
...
reg edrive_clip=0;
always @(posedge clk2x) begin
   local_eig_drive <= piezo_eig_drive_acc[cavity_count] + noise_eig_drive;  // pipeline add just like cav4_elec.v
