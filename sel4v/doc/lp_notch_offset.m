...
fo=3.5606;  % MHz offset of passband mode
%fo=1.8;
fn=fo+0.8;   % MHz offset of nearby mode to be rejected

Al=2*pi*fl./(s+2*pi*fl+2*pi*i*fo);  % low-pass filter, cutoff fl, offset fo

Af=2*pi*fl./(s+2*pi*fl+2*pi*i*fn);  % first order filter, complex-valued pole,
% same bandwidth as the low-pass filter

% low-pass filter evaluated at notch frequency
Al0=2*pi*fl./(-2*pi*i*fn+2*pi*fl+2*pi*i*fo);  % constant complex number
...
T=2/94.286;  % us (LCLS-II-compatible)
z=exp(s*T);
zo=exp(-2*pi*i*fo*T);
zlp=exp(-2*pi*(fl+i*fo)*T)
...
if 1
  Alz_x=(zo-zlp)./(z-1-z.^(-1)*zlp*(zlp-1));
  Afz_x=(zo-zlp)./(z-1-z.^(-1)*zbp*(zbp-1));
  Al0z_x=(zn-1-zn^(-1)*zbp*(zbp-1))/(zn-1-zn^(-1)*zlp*(zlp-1))
  Az_x=Alz_x-Al0z_x*Afz_x;
end
%plot(f,abs(Al),f,abs(Alz),f,abs(Alz_x))
