...
def run_test_bench(setmp_val, coeff_val, lim_val, test_type, in_file, out_file, setmp_step_time=0, setmp_step_val=0, lim_step_time=0, lim_step_val=0, in_i=0, in_q=0):
    """
    ...
    lim = get_reg_info(regmap_fdbk_core, [], 'lim') # Controller upper and lower limits
    ...
    if ((test_type == 2) or (test_type == 3) or (test_type == 4)):
        ...
        setmp['value'][index] = setmp['value'][index] + setmp_step_val  # set X, set Y
        ...
        command = 'vvp -n fdbk_core_tb +vcd +test=' + str(test_type) + ' +in_file=' + in_file + ' +out_file=' + out_file
    elif (test_type == 1):
        command = 'vvp -n fdbk_core_tb +vcd +test=' + str(test_type) + ' +in_file=' + in_file + ' +out_file=' + out_file + ' +in_i=' + str(in_i) + ' +in_q=' + str(in_q)
...
def run_sp_test_bench():
    ...
    # Plot results
    plt.plot(trang*1e6, np.abs(input_mp), label='Input Amplitude', linewidth=2)
    plt.plot(trang*1e6, np.abs(setpoint), label='Set-point Amplitude', linewidth=2)
    plt.plot(trang*1e6, np.abs(error), label='Error Amplitude', linewidth=2)

    # Format plot
    plt.title("Set-point amplitude scaling test-bench", fontsize=40, y=1.01)
    ...
    phase_set = np.array([0.0, 45.0, 90.0, 135.0, 180.0, -180.0, -135.0]) # Degrees
    ...
    for i, phase in enumerate(phase_set):
        ...
        setmp_val = [amplitude_set*1.646760258/2, phase_set_rad[i]*(2**17-1)*2.0/(2*np.pi)]

        # Run Verilog test-bench
        run_test_bench(setmp_val, coeff_val, lim_val, test_type, in_file, out_file, in_i=in_i[i], in_q=in_q[i])
        # Load output file from Verilog test-bench
        data = np.loadtxt(out_file, skiprows=1)
        # Build time vector
        ...
        # Plot results
        plt.plot(trang*1e6, np.imag(input_mp)*360/2**18, label='Input', linewidth=2)
        plt.plot(trang*1e6, np.imag(setpoint)*360/2**18, label='Set-point', linewidth=2)
        plt.plot(trang*1e6, np.imag(error)*360/2**18, label='Error', linewidth=2)

        # Format plot
        title_text = 'Set-point phase scaling test-bench (%d'%(int(phase))+r'$\degree$'+')'
...
def run_prop_test_bench():
    ...
    # Run Verilog test-bench
    run_test_bench(setmp_val, coeff_val, lim_val, test_type, in_file, out_file, setmp_step_time=setmp_step_time,
                   setmp_step_val=setmp_step_val, lim_step_time=100, lim_step_val=lim_step_val)
    ...
    out1 = np.real(fdbk_out[setmp_step_time/2])
    out2 = np.real(fdbk_out[setmp_step_time/2+100/2])
    edge_ind = np.where((np.real(fdbk_out) < out1-2) & (np.real(fdbk_out) > out2+2))[0]

    kp_measured = (fdbk_out[edge_ind[0]-1] - fdbk_out[edge_ind[-1]+1])/-setmp_step_val
    kp_text = r'$\rm k_{\rm p}$'+' set to: %.1f, measured: %.1f' % (kp, np.real(kp_measured))
    print '\nKp: Set to %.3f, measured: %.3f' % (kp, np.real(kp_measured))

    plt.text(1.6, -60000, kp_text, verticalalignment='top', fontsize=30)

    # Plot results
    plt.plot(trang*1e6, np.real(setpoint), label='Set-point', linewidth=2)
    plt.plot(trang*1e6, np.real(fdbk_out), label='Controller Output', linewidth=2)
    plt.plot(trang*1e6, np.real(error), label='Error', linewidth=2)

    # Format plot
    plt.title("Proportional gain test-bench (Amplitude)", fontsize=40, y=1.01)
    plt.xlabel('Time ['+r'${\rm \mu}$' + 's]', fontsize=30)
    plt.ylabel('Amplitude [FPGA counts]', fontsize=30)
    plt.legend(loc='upper right')
    plt.xlim([1.4, 2.5])
    ...
    plt.axvline(x=(setmp_step_time)*Tstep*1e6, color='k', linestyle='--')
    ...
    phase_set_rad = phase_set*2.0*np.pi/360 # Radians
    setmp_val =[amplitude_set*1.646760258/2, phase_set_rad*(2**17-1)*2.0/(2*np.pi)]
    ...
    # Run Verilog test-bench
    run_test_bench(setmp_val, coeff_val, lim_val, test_type, in_file, out_file, setmp_step_time=setmp_step_time,
                   setmp_step_val=setmp_step_val, lim_step_time=100, lim_step_val=lim_step_val, in_i=in_i, in_q=in_q)
    ...
    # Find latency in clock cycles
    out1 = np.imag(fdbk_out[setmp_step_time/2])
    out2 = np.imag(fdbk_out[setmp_step_time/2+100/2])
    edge_ind = np.where((np.imag(fdbk_out) < out1-2) & (np.imag(fdbk_out) > out2+2))[0]

    kp_measured = (fdbk_out[edge_ind[0]-1] - fdbk_out[edge_ind[-1]+1])/-setmp_step_val
    kp_text = r'$\rm k_{\rm p}$'+' set to: %.1f, measured: %.1f' % (kp, np.imag(kp_measured))
    print '\nKp: Set to %.3f, measured: %.3f' % (kp, np.imag(kp_measured))

    plt.text(1.6, -60000*360/2**18, kp_text, verticalalignment='top', fontsize=30)

    # Plot results
    plt.plot(trang*1e6, np.imag(setpoint)*360/2**18, label='Set-point', linewidth=2)
    plt.plot(trang*1e6, np.imag(fdbk_out)*360/2**18, label='Controller Output', linewidth=2)
    plt.plot(trang*1e6, np.imag(error)*360/2**18, label='Error', linewidth=2)
...
def run_int_test_bench():
    ...
    setmp_step_val = 8000
    limit = 30000
    lim_step_val = [limit/1.646760258, limit/1.646760258, -limit/1.646760258, -limit/1.646760258]
    setmp_step_time = 150
    ...
    edge_ind = np.where((np.real(fdbk_out) < out1-1500) & (np.real(fdbk_out) > -limit+1500))[0]

    slope, b = np.polyfit(trang[edge_ind], np.real(fdbk_out[edge_ind]), 1)  # Slope
    ki_measured = Tstep*slope/setmp_step_val
    ki_text = r'$\rm k_{\rm i}$'+' set to: %.4f, measured: %.4f' % (ki, np.real(ki_measured))
    low_lim = fdbk_out[-1]
    limit_text = 'Lower limit set to %d , measured %.1f' % (-limit, np.real(low_lim))

    print '\nKi: Set to %.6f, measured %.6f, factor %.4f' % (ki*1.646760258/2**14, np.real(ki_measured), ki/ki_measured)
    print limit_text

    plt.text(0.25, -10000, ki_text, verticalalignment='top', fontsize=30)
    plt.text(0.25, -13000, limit_text, verticalalignment='top', fontsize=30)
    ...
    plt.plot(trang[edge_ind]*1e6, np.real(fdbk_out[edge_ind]), label='Slope', linewidth=3)
    ...
    edge_ind = np.where((np.imag(fdbk_out) < out1-1500) & (np.imag(fdbk_out) > -limit+1500))[0]

    slope, b = np.polyfit(trang[edge_ind], np.imag(fdbk_out[edge_ind]), 1)  # Slope
    ki_measured = Tstep*slope/setmp_step_val
    ki_text = r'$\rm k_{\rm i}$'+' set to: %.4f, measured: %.4f' % (ki, ki_measured)
    low_lim = fdbk_out[-1]
    limit_text = 'Lower limit set to %d , measured %.1f' % (-limit, np.imag(low_lim))

    print '\nKi: Set to %.6f, measured %.6f, factor %.4f' % (ki*1.646760258/2**14, ki_measured, ki/ki_measured)
    print limit_text

    plt.text(0.25, -10000*360/2**18, ki_text, verticalalignment='top', fontsize=30)
    plt.text(0.25, -15000*360/2**18, limit_text, verticalalignment='top', fontsize=30)
    ...
    # Highlight the region of the output signal where the slope is calculated
    plt.plot(trang[edge_ind]*1e6, np.imag(fdbk_out[edge_ind])*360/2**18, label='Slope', linewidth=3)
...
def run_latency_test_bench():
    ...
    # Run Verilog test-bench
    run_test_bench(setmp_val, coeff_val, lim_val, test_type, in_file, out_file, setmp_step_time=setmp_step_time, setmp_step_val=setmp_step_val, lim_step_time=lim_step_time, lim_step_val=lim_step_val)
    ...
    plt.text(0.5, 20000, latency_text, verticalalignment='top', fontsize=30)

    # Plot results
    plt.plot(trang*1e6, np.real(fdbk_in), label='Controller Input', linewidth=2)
    plt.plot(trang*1e6, np.real(fdbk_out), label='Controller Output', linewidth=2)
