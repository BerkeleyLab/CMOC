import datetime, os, subprocess, getopt, sys, re

def config_romx(dsp_flavor=0, tool_rev=0, board_type=0):
	if isinstance(tool_rev, basestring) and tool_rev.isdigit():
		tool_rev = eval(tool_rev)
	else:
		tool_rev = 0
	if isinstance(dsp_flavor, basestring) and dsp_flavor.isdigit():
	...
	if board_type is None:
		board_type = 0
	elif isinstance(board_type, basestring):
...
#			version_number = 0
		...
		version_number = 0
	try:
		code_is_dirty = 0 if ('nothing' in subprocess.check_output(['git', 'status']).split('\n')[-2]) else 1
	except:
		code_is_dirty = 0
	...
	comments = ['magic', 'dsp flavor', 'year', 'month', 'day', 'hour', 'minute',
		'code is (dirty), () commits after the latest tag',
		'tool rev (%s)'%tool_rev, 'user (%s)'%user, 'board type (%s)'%board_name,
		'version number', 'start of git commit SHA1'] + ['...']*18 + ['end of git commit SHA1']
	...
	config_case = '\n'.join(['''\t5'h%s: data = 8'h%s; // %s'''%(format(i, '02x'),
								     str_config[i * 2: i * 2 + 2],
								     comments[i])
				 for i in range(32)])
...
if __name__=="__main__":
	opts, args = getopt.getopt(sys.argv[1:], 'h:t:f:o:',['HARDWARE=', 'TOOL=', 'DSP_FLAVOR=', 'OUTPUT='])
	dsp_flavor = 2
	tool_rev = '1.0'
	...
	for opt,arg in opts:
		if opt in ('-h', '--HARDWARE'):
			...
			usage()
	f = open(filename, 'w')
	f.write(config_romx(dsp_flavor=dsp_flavor, tool_rev=tool_rev, board_type=board_type))
	f.close()
