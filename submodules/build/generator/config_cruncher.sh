...
tool_ver=0
if [ "$TOOL" = 2 ]; then
    tool_string=`if [ -r "$XILINX_SETTINGS" ]; then . $XILINX_SETTINGS >/dev/null; if [ -r $XILINX/readme.txt ]; then awk 'FNR==1{gsub("\r","");print $0}' $XILINX/readme.txt; elif [ -r $XILINX/fileset.txt ]; then awk '/ version=/{a=$NF; sub("version=","",a); print "XST", a}' $XILINX/fileset.txt; fi; fi`
    tool_ver=`echo $tool_string | awk '{a=match($0,"[0-9]+.[0-9]+"); b=substr($0,a,RLENGTH); split(b,c,"."); print c[1]*16+c[2];}'`
    echo "tool_string=$tool_string  tool_ver=$tool_ver" >&2
fi
...
if [ -d ../.git -o -d .git ]; then
	LATEST_COMMIT_SHA1=`git log | head -n1 | awk '{print $2}' | fold -w 2`
    set +e
