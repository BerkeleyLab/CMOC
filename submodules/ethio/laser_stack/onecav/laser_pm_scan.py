...
from laser_lock_pm_cali import LaserLockPMScan
...
def start_io(addr, port):
    target = LaserLockPMScan(ip=addr, port=port)
    target.get_config_rom().addCallback(target.print_lbnl_rom)
    freq=350.
    interval=1./freq
    from itertools import cycle
    i = cycle(range(-10,10,3))
    t1 = task.LoopingCall(target.pm_cali_lock_loop,i)
    t1.start(15)
