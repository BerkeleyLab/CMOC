...
from laser_phase_cali_one_cav import LaserPhaseCali
...
def start_io(addr, port,argv):
    num = eval(argv[0])
    if (not isinstance(num,int)) or num>3 or num<0: 
        raise ValueError('wrong cav num')
    target = LaserPhaseCali(ip=addr, port=port, cav_num=num)
