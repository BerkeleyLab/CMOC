...
wire [pcw-1:0] pc_d;
reg_delay #(.dw(pcw),.len(11)) pc_del(.clk(clk), .gate(1'b1), .din(pc), .dout(pc_d));
...
// Binary scaling
wire [2:0] scale_d;
reg_delay #(.dw(3),.len(5)) sc_del(.clk(clk), .gate(1'b1), .din(scale), .dout(scale_d));
...
reg_delay #(.dw(36),.len(6)) ab_del(.clk(clk), .gate(1'b1), .din(ab_out), .dout(ab_del_out));
